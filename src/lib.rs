...
pub fn as_ref<U, T: AsRef<U>>(bound: &Bound<T>) -> Bound<&U> {
    match bound {
...
pub fn as_mut<U, T: AsMut<U>>(bound: &mut Bound<T>) -> Bound<&mut U> {
    match bound {
...
pub fn as_pin_ref<'a, U, T: AsRef<U>>(bound: Pin<&'a Bound<T>>) -> Bound<Pin<&'a U>> {
    unsafe { map(as_ref(Pin::get_ref(bound)), |x| Pin::new_unchecked(x)) }
}
pub fn as_pin_mut<'a, U, T: AsMut<U>>(bound: Pin<&'a mut Bound<T>>) -> Bound<Pin<&'a mut U>> {
    unsafe {
...
impl<T> MyBound<T> {
    ...
    pub fn as_ref<U>(&self) -> MyBound<&U>
    where
        T: AsRef<U>,
    {
        MyBound(as_ref(&self.0))
    }
    pub fn as_mut<U>(&mut self) -> MyBound<&mut U>
    where
        T: AsMut<U>,
    {
        MyBound(as_mut(&mut self.0))
    }
    pub fn as_pin_ref<'a, U>(self: Pin<&'a MyBound<T>>) -> MyBound<Pin<&'a U>>
    where
        T: AsRef<U>,
    {
        MyBound(unsafe { map(as_ref(&Pin::get_ref(self).0), |x| Pin::new_unchecked(x)) })
    }
    pub fn as_pin_mut<'a, U>(self: Pin<&'a mut MyBound<T>>) -> MyBound<Pin<&'a mut U>>
    where
        T: AsMut<U>,
    {
        MyBound(unsafe {
